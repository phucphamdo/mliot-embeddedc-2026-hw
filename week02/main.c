...
typedef union
{
    ...
    struct
    {
        ...
        uint16_t ASSIST_LEVEL : 2; // Bit 1-2
        uint16_t LIGHT_BRIGHT : 4; // Bit 3-6
        uint16_t RESERVED : 9;     // Bit 7-15
...
void Battery_Monitor(void (*overheat_cb)(void))
{
    ...
    // Kiểm tra điều kiện nhiệt độ và gọi callback
...
{
    // HỌC VIÊN BẮT ĐẦU VIẾT CODE TỪ ĐÂY
    int stack_drainer[100];
    stack_drainer[0] = 1; // tránh warning unused-variable
    crash_simulation();   // đệ quy vô hạn -> Stack Overflow
